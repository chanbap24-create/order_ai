-- 수금일 유형 추가(익월25·익익월10/15/말) — cl_d(마감시점) 계산에 새 유형 반영.
...
AS $function$
  ...
  base AS (
    SELECT d.client_code, d.client_name, d.business_type,
      ...
      (CASE WHEN f.payment_type IN ('nm5','nm10','nm15','nm20','nm25','nme')
            THEN (date_trunc('month', p_as_of)::date - 1)               -- 익월류: 지난달 말 마감
            WHEN f.payment_type IN ('nnm10','nnm15','nnme')
            THEN ((date_trunc('month', p_as_of) - interval '1 month')::date - 1)  -- 익익월류: 2달 전 말 마감
