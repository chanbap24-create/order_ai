...
- **⚠️ 재고 합산은 단일 정의만**: `inventory_cdv` 생성 컬럼 `stock_bonded`(용마+KCTC보세)·`stock_total`(가용+보세)·`stock_pipeline`(+입고예정) 또는 `app/lib/stock.ts` 헬퍼만 사용. **재고 수식을 인라인으로 다시 쓰지 말 것** (2026-09 리팩터링 `d061faf0`으로 15개 지점 청산). **`bonded_kctc`는 가용재고의 위치 분해 — 합산에 넣으면 이중계상**(데이터 검증 완료). 창고 구조가 바뀌면 DB 컬럼 정의 + stock.ts 두 곳만 수정.
