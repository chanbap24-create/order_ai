  ...
  try {
    ...
    if (data.parsed_items && data.parsed_items.length > 0) {
      ...
      if (item.candidates && item.candidates.length > 0) {
        ...
        console.log('Top 10 후보:');
        item.candidates.slice(0, 10).forEach((c, i) => {
          console.log(`${i+1}. [${c.item_no}] ${c.item_name}`);
          console.log(`   점수: ${c.score}, 신규: ${c.is_new_item}`);
        });
        console.log('');
        ...
        if (marlborough) {
          const rank = item.candidates.findIndex(c => c.item_no === '3A24401') + 1;
          console.log(`✅ 말보로 품목 찾음: ${rank}순위`);
          console.log(`   ${marlborough.item_no} ${marlborough.item_name}`);
