 ...
 * 토큰의 매핑 조회
 */
export function getTokenMapping(token: string): TokenMapping | null {
  try {
 ...
 * 모든 학습된 매핑 조회 (디버깅용)
 */
export function getAllTokenMappings(): TokenMapping[] {
  try {
 ...
 * 학습된 토큰 매핑으로 검색어 확장
 */
export function expandQuery(rawQuery: string, minConfidence: number = 0.5): ExpandedQuery {
