...
export interface SegmentTopItem { item_no: string; name: string; breadth: number; reorder: number; qty: number }
...
interface Agg { clients: Set<string>; qty: number; prices: { p: number; w: number }[]; types: Record<string, number>; countries: Record<string, number>; items: Record<string, { name: string; qty: number; clients: Map<string, number> }> }
const mkAgg = (): Agg => ({ clients: new Set(), qty: 0, prices: [], types: {}, countries: {}, items: {} });

/** 최근 12개월 출고를 업장유형·지역별로 집계해 segment_profiles 갱신. */
export async function refreshSegmentProfiles(): Promise<{ venues: number; businessTypes: number; regions: number; lines: number }> {
  ...
  const [clients, venues, wines, ships] = await Promise.all([
    pageAll<{ client_code: string; business_type: string | null; address: string | null }>('client_details', 'client_code,business_type,address', (q) => q.eq('client_type', 'wine')),
  ...
  const btOf = new Map<string, string>(); for (const c of clients) if (c.business_type) btOf.set(String(c.client_code), c.business_type);
  ...
  const addTo = (m: Map<string, Agg>, key: string, code: string, s: typeof ships[number], w?: typeof wines[number]) => {
    ...
    const nm = w?.item_name_kr || s.item_name || s.item_no; const it = a.items[s.item_no] || (a.items[s.item_no] = { name: nm, qty: 0, clients: new Map() }); it.qty += qty; it.clients.set(code, (it.clients.get(code) || 0) + 1);
  };
  for (const s of ships) {
    ...
    const bt = btOf.get(code); if (bt) addTo(byBt, bt, code, s, w);
  ...
  const toRow = (segmentType: string, key: string, label: string, a: Agg) => {
    ...
    const top_items = Object.entries(a.items).map(([item_no, v]) => {
      const reorder = [...v.clients.values()].filter((c) => c >= 2).length; // 2회+ 재구매 거래처 수(검증 신호)
      return { item_no, name: v.name, breadth: v.clients.size, reorder, qty: v.qty };
    }).sort((x, y) => y.reorder - x.reorder || y.breadth - x.breadth || y.qty - x.qty).slice(0, 20);
    return {
  ...
  const rows = [
    ...
    ...[...byBt.entries()].filter(([, a]) => a.clients.size >= 5).map(([k, a]) => toRow('business_type', k, k, a)),
  ...
  return { venues: byVenue.size, businessTypes: byBt.size, regions: byRegion.size, lines: ships.length };
}

/** 추천용 단건 조회. */
export async function getSegmentProfile(segmentType: 'venue' | 'business_type' | 'region', key: string): Promise<SegmentProfile | null> {
  if (!key) return null;
