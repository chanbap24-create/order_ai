...
export function renderHeader(slide: Slide, data: SlideData) {
  ...
  // 상단 타이틀 = 와이너리명 + 지역명(부제). 와인 풀네임은 아래 카드에.
  ...
  const nameRuns: PptxGenJS.TextProps[] = [
    { text: nameEnClean ? nameKrClean + "\n" : nameKrClean, options: { fontSize: krFontSize, fontFace: FONT_MAIN, color: C.BURGUNDY_DARK, bold: true } },
  ];
  if (nameEnClean) {
    nameRuns.push({ text: nameEnClean, options: { fontSize: enFontSize, fontFace: FONT_EN, color: C.TEXT_SECONDARY, italic: true } });
  }
