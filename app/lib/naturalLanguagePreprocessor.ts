...
// 별칭 캐시 로드 (양방향)
async function loadAliasCache(): Promise<{
...
// 별칭 확장 (양방향)
export async function expandAliases(text: string, debug = false): Promise<string> {
  const { forward: aliases, reverse: reverseAliases } = await loadAliasCache();
  let expanded = text;
...
// 통합 전처리 함수
export async function preprocessNaturalLanguage(text: string): Promise<string> {
  let processed = text;
  ...
  // 2. 별칭 확장 (가장 먼저!)
  processed = await expandAliases(processed);
...
export async function preprocessWithDebug(text: string): Promise<PreprocessResult> {
  ...
  current = await expandAliases(current);
