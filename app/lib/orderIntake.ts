...
[거래처(client_hint)]
- 기본: 화면 상단 채팅방 제목에서 상호(가게/업장 이름)만 뽑는다.
- 사람 이름·직함(소믈리에/과장/대표/사장님/실장 등)은 제거.
  예: "에피세리 꼴라주 김정훈 소믈리에" → "에피세리 꼴라주"
- 단, 발주 본문에 보낼 곳(배송지)을 명시하면 그 상호를 채팅방 제목보다 "우선"한다.
  (한 담당자가 여러 거래처를 관리해, 어디로 보낼지 본문에 적는 경우가 있다.)
  명시 형태 예: "OO로 발주", "OO으로 보내주세요", "OO으로 배송", "OO에 넣어주세요".
  예: 제목 "파티나 이성윤 (매쎄)" + 본문 "매쎄로 ... 발주 부탁" → client_hint "매쎄"
  주의: "내일로/택배로/퀵으로" 처럼 날짜·배송수단은 거래처가 아니므로 제외.
