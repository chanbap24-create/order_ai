...
// ✅ Glass 코드 패턴: 3~4자리 + 슬래시 + 1~3자리
const GLASS_CODE_RE = /\d{3,4}\/\d{1,3}[A-Z]?/i;
...
export function parseGlassItemsFromMessage(text: string): ParsedGlassItem[] {
  ...
  for (const line of lines) {
    ...
    const codePattern = /^(\d{3,4}\/\d{1,3}[A-Z]?)\s*\(([^)]+)\)\s*[:：]?\s*(\d+)\s*(잔|개|병|박스|box)?/i;
    ...
    // "0884/0 6", "0447/07 12", "4100/00R 6", "330/07 6" ← 3자리도 허용
    const codeOnlyPattern = /^(\d{3,4}\/\d{1,3}[A-Z]?)\s+(\d+)\s*(잔|개|병|박스|box)?$/i;
    ...
    // "크로스비 0425/0 6", "스시소라 330/07 12" ← 3자리도 허용
    const textCodePattern = /^(.+?)\s+(\d{3,4}\/\d{1,3}[A-Z]?)\s+(\d+)\s*(잔|개|병|박스|box)?$/i;
    ...
    if (m2) {
      ...
      // ✅ 품목명 안에 코드 패턴이 숨어있는지 확인 (3~4자리 허용)
      const embeddedCode = name.match(/(\d{3,4}\/\d{1,3}[A-Z]?)/i);
      if (embeddedCode) {
        items.push({
          raw: line,
