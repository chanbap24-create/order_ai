...
export async function GET(request: NextRequest) {
  try {
    ...
    if (wineStatus || status === 'new') {
      const wines = await getNewWinesWithStatus({ status, search, wineStatus });
      return NextResponse.json({ success: true, data: wines });
    }

    const wines = await getWines({ status, search, country });
    return NextResponse.json({ success: true, data: wines });
  } catch (e) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    await upsertWine(body);
    await logChange('wine_created', 'wine', body.item_code, { item_name: body.item_name_kr });
