...
export function scoreName(q: any, name: any) {
  ...
  if (nameAlias) {
    ...
    let commonAlias = 0;
    for (const ch of aChars) if (aliasChars.has(ch)) commonAlias++;
    ...
    let commonMain = 0;
    for (const ch of aChars) if (mainChars.has(ch)) commonMain++;
 ...
 */
export function extractVintage(itemNo: string): number | null {
  const code = String(itemNo || '');
  if (code.length < 4) return null;

  const vintageStr = code.substring(2, 4);
...
/**
 * 품목명에서 빈티지 제거 (비교용). 예: "샤블리 (2021)" → "샤블리"
 */
export function removeVintageFromName(name: string): string {
  return String(name || '')
    .replace(/^[A-Z]{2}\s+/, '') // 앞 접두어 제거 (AR, VG, MR 등)
