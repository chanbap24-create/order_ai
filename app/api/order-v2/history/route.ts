...
import { getSellingUnitPrice } from '@/app/lib/priceUtils';
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const { data, error } = await supabase
      .from(shipTable)
      .select('item_no, item_name, ship_date, unit_price, selling_price, supply_amount, quantity')
      .eq('client_code', clientCode)
      .order('ship_date', { ascending: false });
    if (error) throw error;

    const getUnitPrice = (row: any): number => {
      return getSellingUnitPrice(row.unit_price || 0, row.selling_price || 0, row.supply_amount || 0, row.quantity || 1);
    };
