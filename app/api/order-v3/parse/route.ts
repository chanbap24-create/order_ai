...
// 발주 v3 — 추출(Claude) → 매칭(matchLineV3). CDV(와인)/DL(글라스) 탭 지원.
export async function POST(req: NextRequest) {
  try {
    const session = await getSession();
    if (!session) return NextResponse.json({ error: '인증이 필요합니다.' }, { status: 401 });

    const { order_text, client_code, from_image, tab } = await req.json();
    ...
    const result = await parseOrderV3(order_text, client_code || null, { fromImage: !!from_image, tab: tab === 'DL' ? 'DL' : 'CDV' });
