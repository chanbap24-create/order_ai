...
export async function GET(req: NextRequest) {
  try {
    ...
    // ═══ Glass: glass_client_carryover 기반 거래처 목록 (Wine과 동일 패턴) ═══
    if (isGlass) {
      // 1. 담당자의 거래처 목록 (glass_client_carryover.manager)
      const { data: clientList, error: clErr } = await supabase
        .from('glass_client_carryover')
        .select('client_code, client_name, carryover_amount')
        .eq('manager', manager);
      ...
      const allCodes = clientList.map(c => c.client_code);
      const carryoverMap = new Map<string, { name: string; carry: number }>();
      for (const c of clientList) {
        carryoverMap.set(c.client_code, { name: c.client_name, carry: c.carryover_amount || 0 });
      }
      ...
      // 3. 이름 기반 추가 출고 (client_code 다르지만 client_name 동일)
      const nameSet = new Set(clientList.map(c => c.client_name).filter(Boolean));
      for (const name of nameSet) {
        let from = 0;
        while (true) {
          const { data, error } = await supabase
            .from('glass_shipments')
            .select('client_code, client_name, supply_amount, tax_amount, total_amount')
            .eq('client_name', name)
          ...
          if (!data || data.length === 0) break;
          // 해당 이름의 첫 번째 코드에 합산
          const mainCode = clientList.find(c => c.client_name === name)?.client_code;
          if (mainCode) {
            for (const r of data) {
              const cur = periodSalesMap.get(mainCode) || { supply: 0, tax: 0, total: 0 };
              cur.supply += r.supply_amount || 0;
              cur.tax += r.tax_amount || 0;
              cur.total += r.total_amount || 0;
              periodSalesMap.set(mainCode, cur);
            }
          }
      ...
      for (const name of nameSet) {
        ...
        while (true) {
          const { data, error } = await supabase
            .from('glass_payments')
            .select('client_code, client_name, amount')
            .eq('client_name', name)
            .not('client_code', 'in', `(${allCodes.join(',')})`)
            .gte('payment_date', startDate).lte('payment_date', endDate)
            .range(from, from + 999);
          ...
          if (mainCode) {
            for (const r of data) {
              periodPayMap.set(mainCode, (periodPayMap.get(mainCode) || 0) + (r.amount || 0));
            }
          }
          if (data.length < 1000) break;
          from += 1000;
      ...
      const clients: any[] = [];
      for (const c of clientList) {
        const carry = c.carryover_amount || 0;
        const ps = periodSalesMap.get(c.client_code);
        const pSupply = ps?.supply || 0;
        const pTax = ps?.tax || 0;
        const pTotal = ps?.total || 0;
        const pPayment = periodPayMap.get(c.client_code) || 0;
        const outstanding = carry + pTotal - pPayment;
