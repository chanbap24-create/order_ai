...
function RemoteSyncButton() {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
      ...
      >
        {isActive ? (
          <><span style={{ display: 'inline-block', width: 12, height: 12, border: '2px solid rgba(255,255,255,0.3)', borderTopColor: 'white', borderRadius: '50%', animation: 'spin 0.6s linear infinite' }} /> {message}</>
        ) : (
          <><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2z"/><path d="M8 12l4 4 4-4M12 8v8"/></svg> 원격 동기화</>
        )}
      </button>
      {status === 'done' && <span style={{ fontSize: 11, color: '#059669', fontWeight: 600 }}>{message}</span>}
      {status === 'error' && <span style={{ fontSize: 11, color: '#dc2626', fontWeight: 600 }}>{message}</span>}
    </div>
  );
}
