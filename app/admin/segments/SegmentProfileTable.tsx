...
const TC: Record<string, string> = { '스파클링': 'var(--status-warning)', '화이트': '#ca8a04', '레드': '#991b1b', '로제': '#db2777', '주정강화': '#7c3aed', '스위트': '#e11d48', '기타': '#9ca3af' };
  ...
  return (
    <div>
      <div style={{ display: 'flex', width: 150, height: 14, borderRadius: 3, overflow: 'hidden', marginBottom: 3 }}>
        {entries.map(([t, v]) => (
          <span key={t} title={`${t} ${Math.round(v * 100)}%`} style={{ width: `${v * 100}%`, background: TC[t] || 'var(--neutral-200)' }} />
        ))}
      </div>
...
export function SegmentProfileTable({ profiles }: { profiles: SegmentProfile[] }) {
  ...
  return (
    <div style={{ overflowX: 'auto', background: '#fff', border: '1px solid #e7e3df', borderRadius: 12 }}>
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12.5, minWidth: 720 }}>
        ...
        <tbody>
          {profiles.map((p) => (
            <tr key={p.segment_type + p.segment_key} style={{ borderTop: '1px solid var(--border-default)' }}>
