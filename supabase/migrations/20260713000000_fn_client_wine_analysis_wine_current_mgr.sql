-- 세일즈 분석(fn_client_wine_analysis) — 와인 담당 필터도 '현재 담당(client_details.manager)'으로 변경.
--  · 종전: 와인만 s.manager(출고당시 담당) 필터가 남아 어드민 매출분석(현재담당)과 어긋남
--    (예: 조성재 와인 2026 — 출고당시 637.6M vs 현재담당 654.9M).
--  · 변경: 와인도 s.client_code IN (client_details wine manager=담당) 로 스코프.
--    글라스(glass_clients 스코프)·이관 컷오프는 20260712020000 그대로.
--  나머지 집계(rev_expr/할인율/by_* /prev 기간)는 기존과 동일.
CREATE OR REPLACE FUNCTION public.fn_client_wine_analysis(p_type text, p_manager text DEFAULT ''::text, p_department text DEFAULT ''::text, p_client text DEFAULT ''::text, p_start text DEFAULT ''::text, p_end text DEFAULT ''::text)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
BEGIN
  ...
  -- 담당 필터: 와인·글라스 모두 '현재 담당' 거래처 코드로 스코프(어드민 매출분석과 동일 정책).
  IF p_manager <> '' THEN
    IF is_wine THEN
      where_clause := where_clause || format(' AND s.client_code IN (SELECT client_code FROM client_details WHERE client_type = ''wine'' AND manager = %L)', p_manager);
    ELSE
      where_clause := where_clause || format(' AND s.client_code IN (SELECT client_code FROM glass_clients WHERE manager = %L)', p_manager);
    END IF;
  END IF;
  ...
  -- 글라스: 2025-08-01 전산이관 전(이월) 제외.
  IF NOT is_wine THEN where_clause := where_clause || ' AND s.ship_date::date >= ''2025-08-01''::date'; END IF;
