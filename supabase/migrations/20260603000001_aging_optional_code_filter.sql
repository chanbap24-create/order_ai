-- 수금 마커 등 "소수 거래처만" 필요한 호출을 위해 aging RPC에 선택적 p_codes 필터 추가.
-- p_codes IS NULL  → 기존과 100% 동일(전체 거래처).
-- p_codes 지정     → 해당 거래처만 계산(client CTE에서 prune → 17배↑ 빠름).
-- 미팅 달력 마커는 promised_date+금액이 설정된 소수 거래처만 표시하므로 전체 aging 불필요.
-- 시그니처 변경(2-arg→3-arg, DEFAULT)이라 DROP 후 재생성. 기존 2-arg 호출은 그대로 동작.

DROP FUNCTION IF EXISTS public.calc_glass_aging(text, date);
CREATE OR REPLACE FUNCTION public.calc_glass_aging(p_manager text, p_as_of date DEFAULT CURRENT_DATE, p_codes text[] DEFAULT NULL)
...
cl AS (
  ...
  WHERE manager = p_manager AND (client_name IS NULL OR UPPER(client_name) NOT LIKE '(X)%')
    AND (p_codes IS NULL OR client_code = ANY(p_codes))
  UNION
...
CREATE OR REPLACE FUNCTION public.calc_wine_aging(p_manager text, p_as_of date DEFAULT CURRENT_DATE, p_codes text[] DEFAULT NULL)
 RETURNS TABLE(client_code text, client_name text, net_balance bigint, b_cur bigint, b_m1 bigint, b_m2 bigint, b_m3 bigint, oldest_unpaid_date date, last_payment_date date, last_payment_amount bigint, paid_90d bigint, overdue bigint)
 LANGUAGE sql STABLE SECURITY DEFINER
AS $function$
WITH
