-- 수금일정표: 익월 결제(nm5/nm10/nm15/nm20/nme) 전체의 이월 기준일을 '전월말'로 보정.
-- 기존엔 nme만 보정했으나, 익월 N일도 마감일(당월20일) 탓에 이번달 출고가 이월로 잡혀
-- 이번달 실적이 당월 결제로 표시되는 문제 동일 → 익월 계열 전체로 일반화.
-- (eom/prepay/미지정은 기존 마감일 유지)
...
RETURNS TABLE(
  ...
  promised_date date, promised_amount bigint
)
LANGUAGE sql STABLE SECURITY DEFINER AS $function$
  ...
  base AS (
    SELECT d.client_code, d.client_name, d.business_type,
      ...
      -- 익월 결제(nm*/nme): 이번달 출고는 익월 결제분 → 이월 기준일을 전월말로
