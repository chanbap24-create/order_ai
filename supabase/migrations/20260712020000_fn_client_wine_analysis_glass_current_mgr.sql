-- 세일즈 분석(fn_client_wine_analysis) — 글라스 담당 필터를 '현재 담당(glass_clients.manager)'으로 변경.
--  · 종전: s.manager(출고당시 담당) 필터 → 재배정 미반영. 어드민/거래처현황과 어긋남.
--  · 변경: 글라스는 s.client_code IN (glass_clients.manager=담당) 로 스코프(과거 매출도 현재 담당 귀속).
--  · 글라스는 2025-08-01 전산이관 전(이월) 출고 제외 추가(어드민 매출분석과 동일 기준).
...
BEGIN
  ...
  -- 담당 필터: 와인=출고 manager, 글라스=현재 담당(glass_clients.manager) 거래처로 스코프.
  IF p_manager <> '' THEN
    IF is_wine THEN
      where_clause := where_clause || format(' AND s.manager = %L', p_manager);
    ELSE
