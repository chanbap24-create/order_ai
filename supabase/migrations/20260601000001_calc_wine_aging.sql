-- 까브드뱅(와인) 미수금 연령 분석 (달력 월 경과: 당월/1·2·3개월+).
-- 채무(이월@ref_date + 출고 total_amount)와 credit(수금 + 반품/선수금 등 음수)을
-- FIFO 매칭해 남은 미수를 월 경과 버킷으로 분류. + 수금 상황(최근수금일/금액, 최근90일 수금).
-- net_balance = calc_wine_outstanding 의 outstanding 과 원 단위 일치. 버킷은 항상 >= 0.
-- 월경과 = (as_of 연*12+월) - (채무일 연*12+월).  0=당월,1,2,>=3=3개월+
...
ORDER BY COALESCE(a.b_m3,0) DESC, net_balance DESC;
$function$;
