-- fn_client_analysis 에 managerBrands 필드 통합
--
-- 기존: client-analysis API 가 fn_client_analysis + fn_manager_brands 를
--       병렬로 호출. Promise.all 이지만 어쨌든 두 RPC 모두 shipments 를
--       필터링(동일 WHERE)해서 각자 스캔함.
-- 신규: fn_client_analysis 의 filtered CTE 를 MATERIALIZED 로 공유해서
--       brand_agg + mgr_brand_agg + mgr_biz_agg 까지 한 번의 스캔으로
--       처리. RPC 한 번만 호출하면 된다.
--
-- API 측에서는 fn_manager_brands 호출 제거. fn_client_analysis 가 반환하는
-- managerAnalysis[i].brands / bizClients 필드를 그대로 사용.
...
BEGIN
  ...
  EXECUTE format(
    'WITH filtered AS MATERIALIZED (SELECT * FROM %I %s),
     summary AS (
     ...
     -- 매니저별 브랜드 집계 (기존 fn_manager_brands 로직 통합)
     mgr_brand_rev AS (
       SELECT
     ...
     mgr_brand_top AS (
       SELECT mgr,
              COALESCE(json_agg(json_build_object(''brand'', brand, ''revenue'', rev) ORDER BY rev DESC) FILTER (WHERE rn <= 10), ''[]''::json) AS brands
       FROM (
         SELECT mgr, brand, rev, ROW_NUMBER() OVER (PARTITION BY mgr ORDER BY rev DESC) AS rn
         FROM mgr_brand_rev WHERE brand IS NOT NULL
       ) r
       GROUP BY mgr
     ),
     mgr_biz_agg AS (
     ...
     SELECT json_build_object(
       ...
       ''managerAnalysis'', (SELECT COALESCE(json_agg(row_to_json(m2) ORDER BY m2.revenue DESC), ''[]''::json) FROM (
         SELECT ma.manager, ma.client_count, ma.revenue, ma.discount_rate,
                COALESCE(mbt.brands, ''[]''::json) AS brands,
                COALESCE(mbzt.biz_clients, ''[]''::json) AS "bizClients"
         FROM mgr_agg ma
         LEFT JOIN mgr_brand_top mbt ON mbt.mgr = ma.manager
         LEFT JOIN mgr_biz_top mbzt ON mbzt.mgr = ma.manager
       ) m2),
       ''countryAnalysis'', (SELECT COALESCE(json_agg(row_to_json(ca)), ''[]''::json) FROM country_agg ca)
     )',
    tbl, where_clause,
    inv_tbl,
    p_type, p_type, p_type, p_type,  -- brand_agg 4개
    p_type, p_type,                   -- mgr_brand_rev 2개
    inv_tbl, inv_tbl, inv_tbl         -- mgr_agg + country_agg + client_discount
  ) INTO result;
