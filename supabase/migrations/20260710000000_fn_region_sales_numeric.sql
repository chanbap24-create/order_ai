-- 지역별 매출 RPC: 매출 계산을 numeric으로 캐스팅해 정수 오버플로 방지.
-- (2025-08 이전 포맷은 selling_price=총액이라 ×수량 시 int 범위(21억) 초과 → 작년 지역별매출 조회가 에러)
      ...
      select s.client_code,
        ...
        case
          when coalesce(s.selling_price,0)=0 and coalesce(s.supply_amount,0)=0 then 0
            ...
            (case
              when abs(coalesce(s.quantity,0)::numeric) <= 1 then (case when abs(coalesce(s.selling_price,0)::numeric)>0 then abs(coalesce(s.selling_price,0)::numeric) else abs(coalesce(s.unit_price,0)::numeric) end)
              when abs(coalesce(s.supply_amount,0)::numeric)>0 and abs(coalesce(s.supply_amount,0)::numeric)>abs(coalesce(s.selling_price,0)::numeric) and abs(abs(coalesce(s.selling_price,0)::numeric)*abs(coalesce(s.quantity,0)::numeric) - abs(coalesce(s.supply_amount,0)::numeric)) < 100 then abs(coalesce(s.supply_amount,0)::numeric)
              when abs(coalesce(s.selling_price,0)::numeric) > abs(coalesce(s.unit_price,0)::numeric)*2 and abs(coalesce(s.unit_price,0)::numeric)>0 then abs(coalesce(s.selling_price,0)::numeric)
              when abs(coalesce(s.selling_price,0)::numeric)>0 then (case when abs(coalesce(s.selling_price,0)::numeric)>abs(coalesce(s.unit_price,0)::numeric) then abs(coalesce(s.selling_price,0)::numeric) else abs(coalesce(s.selling_price,0)::numeric)*abs(coalesce(s.quantity,0)::numeric) end)
              else abs(coalesce(s.unit_price,0)::numeric)*abs(coalesce(s.quantity,0)::numeric)
            end)
        end as rev
