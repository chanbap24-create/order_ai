...
async function main() {
  ...
  // ---- 변형: 가격 범위 게이트 모드 비교(하드컷 vs 소프트감점 vs 무시 vs 밴드확대) ----
  const VARIANTS: { name: string; priceGate: 'hard' | 'soft' | 'off'; band?: number }[] = [
    { name: '하드게이트(현행)', priceGate: 'hard' },
    { name: '소프트게이트', priceGate: 'soft' },              // 범위 밖 = 제외 대신 거리비례 감점(−35)
    { name: '게이트off', priceGate: 'off' },                 // 가격 무시(순수 점수)
    { name: '밴드40%(하드)', priceGate: 'hard', band: 0.4 }, // 범위 자체를 넓혀 하드컷
  ];

  console.log(`\n[백테스트] CDV · 신규제안 — 가격 게이트 모드 비교(세그먼트 포함)`);
  console.log(`  컷오프 T=${cutoff} · 학습창=[${trainFrom}, ${cutoff}) · 정답창=[${cutoff}, ${postTo}] · 최소이력=${minHistory}${limit ? ` · 상한=${limit}` : ''}`);
  console.log(`  기본밴드=±${Math.round(o.priceBandPct * 100)}% · 견적학습/전환은 데이터부족(2주치)이라 OFF.\n`);
  ...
  for (const [code, e] of byClient.entries()) {
    ...
    for (let vi = 0; vi < VARIANTS.length; vi++) {
      ...
      const scored = scoreRecommendations({
        ...core,
        priceBandPct: v.band ?? o.priceBandPct,
        priceGate: v.priceGate,
      });
  ...
  console.log(`평가 거래처: ${evaluated}곳(세그먼트매칭 ${segClients}) · 평균 정답 ${(sumPositives / evaluated).toFixed(1)}개 · 유니버스 ${Math.round(sumUniverse / evaluated)}개`);
  ...
  // 현행(하드) 대비 게이트 변형 델타(hit@10)
  const base = aggVar[0][0].hit; // 하드게이트(현행)
  console.log(`\n  ▶ 현행(하드) hit@10 = ${pct(base)}% · 게이트 변형 델타(hit@10, %p):`);
